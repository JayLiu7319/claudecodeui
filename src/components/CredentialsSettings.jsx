...
function CredentialsSettings() {
  ...
  const deleteGithubCredential = async (credentialId) => {
    if (!confirm(t('settings.githubTokens.deleteConfirm'))) return;
    ...
    <div className="space-y-8">
      ...
      {newlyCreatedKey && (
        <div className="p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-lg">
          <h4 className="font-semibold text-yellow-500 mb-2">{t('settings.apiKeys.saveKeyAlert')}</h4>
          <p className="text-sm text-muted-foreground mb-3">
            {t('settings.apiKeys.saveKeyDescription')}
          </p>
      ...
      <div>
        ...
        <div className="mb-4">
          ...
          >
            {t('settings.apiKeys.apiDocumentation')}
        ...
        {showNewKeyForm && (
          <div className="mb-4 p-4 border rounded-lg bg-card">
            ...
            <div className="flex gap-2">
              <Button onClick={createApiKey}>{t('settings.apiKeys.create')}</Button>
        ...
        <div className="space-y-2">
          {apiKeys.length === 0 ? (
            <p className="text-sm text-muted-foreground italic">{t('settings.apiKeys.noKeysCreated')}</p>
          ) : (
            apiKeys.map((key) => (
              <div
                key={key.id}
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-semibold">{t('settings.credentials.title')}</h3>
          </div>
          <Button
            size="sm"
          ...
          >
            <Plus className="h-4 w-4 mr-1" />
            {t('settings.githubTokens.addToken')}
          </Button>
        </div>
        ...
        {showNewGithubForm && (
          <div className="mb-4 p-4 border rounded-lg bg-card space-y-3">
            <Input
              placeholder={t('settings.githubTokens.tokenNamePlaceholder')}
              value={newGithubName}
            ...
            <div className="relative">
              <Input
                ...
                placeholder={t('settings.githubTokens.tokenValuePlaceholder')}
            ...
            <Input
              placeholder={t('settings.credentials.descriptionOptional')}
              value={newGithubDescription}
              onChange={(e) => setNewGithubDescription(e.target.value)}
            />

            <div className="flex gap-2">
              <Button onClick={createGithubCredential}>{t('settings.githubTokens.addToken')}</Button>
            ...
            >
              {t('settings.credentials.howToCreateToken')}
            </a>
          </div>
        )}
        ...
        <div className="space-y-2">
          {githubCredentials.length === 0 ? (
            <p className="text-sm text-muted-foreground italic">{t('settings.githubTokens.noTokensAdded')}</p>
          ) : (
            githubCredentials.map((credential) => (
              ...
              >
                <div className="flex-1">
                  ...
                  <div className="text-xs text-muted-foreground mt-1">
                    {t('settings.githubTokens.added')} {new Date(credential.created_at).toLocaleDateString()}
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <Button
                  ...
                  >
                    {credential.is_active ? t('settings.apiKeys.active') : t('settings.apiKeys.inactive')}
                  </Button>
                  <Button
                    size="sm"
                    variant="ghost"
      ...
      <div className="pt-6 border-t border-border/50">
        <div className="flex items-center justify-between text-xs italic text-muted-foreground/60">
          ...
          {updateAvailable && latestVersion && (
            ...
            >
              <span className="text-[10px]">{t('settings.credentials.updateAvailable', { version: latestVersion })}</span>
